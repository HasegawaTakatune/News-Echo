...
const fetchNews = async (page = 1) => {
  loading.value = true;
  try {
    const res = await api.get<{ data: NewsItem[]; current_page: number; last_page: number }>("/news", { page: String(page) });
  ...
  <div class="dashboard">
    ...
    <main class="main">
      ...
      <div v-if="pagination.last_page > 1" class="pagination">
        <button :disabled="pagination.current_page <= 1" @click="fetchNews(pagination.current_page - 1)">前へ</button>
        <span>{{ pagination.current_page }} / {{ pagination.last_page }}</span>
        <button :disabled="pagination.current_page >= pagination.last_page" @click="fetchNews(pagination.current_page + 1)">次へ</button>
      </div>
    </main>
  </div>

  <ArticleModal v-model="showModal" :news="editNews" :user-id="user?.id" @save="onSave" />
